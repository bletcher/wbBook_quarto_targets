...
ggplot(propNegSRS |> filter(n > 50), aes(seasonGG, propNeg, color = riverGG, group = riverGG, shape = riverGG)) +
...
Figure 5
```{r}
ggplot(propNegSRsN |> filter(n > 15, seasonGG == "Summer", speciesGG == "Brook trout", year > 2001), aes(year, propNeg, color = riverGG, shape = riverGG)) +
  geom_point(aes(size = n)) +
  geom_line() +
  labs(x = "Year", y = "Proportion of observations with negative growth in mass") +
  scale_color_discrete() +
  theme_publication(base_size = 17)
```
...
```{r}
#| label: negGrowth_seenAgain


negGr_seenNotSeen = cd1 |> 
  filter(samplesBeforeLast > 0) |>
  group_by(speciesGG, riverGG, seasonGG, lastObs = samplesBeforeLast <= 1, negGrowth) |> 
  summarize(
    meanGR = mean(grWeight, na.rm = TRUE),
    n = n()
  ) |> 
  filter(!is.na(negGrowth), n > 10)

ggplot(negGr_seenNotSeen |> filter(!is.na(negGrowth), riverGG == "West Brook"), aes(negGrowth, n, fill = lastObs)) +
  geom_col(position = "dodge") +
  theme_publication() +    
  facet_grid(speciesGG~seasonGG)


negGr_seenNotSeen_wide <- negGr_seenNotSeen |> ungroup() |> select(-meanGR) |> spread(lastObs, n, fill = 0) |>
  mutate(ratio = `TRUE`/`FALSE`)

ggplot(negGr_seenNotSeen_wide |> filter(!is.na(negGrowth), seasonGG != "Spring"), aes(speciesGG, ratio, fill = negGrowth)) +
  geom_col(position = "dodge") +
  scale_y_continuous("Ratio of number of fish not seen again to seen again") +
...
ggplot(negGr_seenNotSeenYearCD |> filter(!is.na(negGrowth), riverGG == "West Brook", year > 2001, year < 2008, seasonGG == "Summer"), aes(negGrowth, n, fill = lastInterval)) +
  geom_col(position = "dodge") +
  theme_publication() +    
  facet_grid(speciesGG ~ seasonGG + year)

ggplot(negGr_seenNotSeenYearCD |> filter(!is.na(negGrowth), riverGG == "West Brook", year >= 2008, year < 2014, seasonGG == "Summer"), aes(negGrowth, n, fill = lastInterval)) +
  geom_col(position = "dodge") +
  theme_publication() +    
  facet_grid(speciesGG ~ seasonGG + year)
...
ggplot(negGr_seenNotSeenYearCD_wide |> filter(!is.na(negGrowth), riverGG == "West Brook", year > 2001, year < 2014, seasonGG == "Summer", speciesGG == "Brook trout", is.finite(ratio)), aes(speciesGG, ratio, fill = negGrowth)) +
  geom_col(position = "dodge") +
  scale_y_continuous("Ratio of number of fish not seen again to seen again") +
  scale_x_discrete("Species") +
  theme_publication(base_size = 17) +    
  facet_grid(riverGG ~ seasonGG + year)
...
ggplot(negGr_seenNotSeenYearCD_wide |> filter(!is.na(negGrowth), speciesGG == "Brook trout", seasonGG == "Summer"), 
  ...
  scale_y_continuous("Number of fish seen again (triangle) and number of fish not seen again (circle)") +
  scale_x_continuous("Year") +
  theme_publication(base_size = 17) + 
  ggtitle("Brook trout") +
  facet_wrap(riverGG ~ seasonGG)
...
# add error bars ymax and ymin
ggplot(negGr_seenNotSeenYearCD_wideMeans |> filter(seasonGG == "Summer"), aes(negGrowth, meanRatio, fill = negGrowth)) +
  geom_col(position = "dodge") +
