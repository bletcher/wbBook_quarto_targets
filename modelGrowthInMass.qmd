...
# )
# save(hpPropNegBKT, file = "./data/out/hyperParam/hpPropNegBKT_250.RData")
load("./data/out/hyperParam/hpPropNegBKT_250.RData")
hpPropNegBKT[1:10,]
...
# )
# save(propNegBKT, file = "./data/out/hyperParam/XGBpropNegBKT_250.RData")
load("./data/out/hyperParam/XGBpropNegBKT_250.RData")
...
#   early_stopping_roundsIn = 250
# )
# save(hpPropNegBNT, file = "./data/out/hyperParam/hpPropNegBNT_250.RData")
load("./data/out/hyperParam/hpPropNegBNT_250.RData")
hpPropNegBNT[1:10,]
...
# )
# save(propNegBNT, file = "./data/out/hyperParam/XGBpropNegBNT_250.RData")
load("./data/out/hyperParam/XGBpropNegBNT_250.RData")
...
#   early_stopping_roundsIn = 250
# )
# save(hpPropNegATS, file = "./data/out/hyperParam/hpPropNegATS_250.RData")
load("./data/out/hyperParam/hpPropNegATS_250.RData")
hpPropNegATS[1:10,]
...
# )
# save(propNegATS, file = "./data/out/hyperParam/XGBpropNegATS_250.RData")
load("./data/out/hyperParam/XGBpropNegATS_250.RData")
...
lastSampleGT1_data <-  cd1 |>
  mutate(lastSampleGT1 = samplesBeforeLast > 1) |> 
  filter(!is.na(negGrowth)) |> 
  group_by(speciesGG, riverGG, seasonGG, lastSampleGT1) |> 
  summarize(
 ...
 #  ggplot(tmp |> filter(n > 2, speciesGG == "Brook trout"), aes(lastSampleGT1, meanGR)) +
 #    geom_errorbar(aes(x = lastSampleGT1, ymin = meanGR, ymax = ymax), width = 0.2) +
 #    geom_col() +
 #    theme_publication() +    
 #    facet_grid(riverGG~seasonGG , scales = "free")
 #  
 # ggplot(tmp |> filter(n > 2, speciesGG == "Brown trout"), aes(lastSampleGT1, meanGR)) +
 #    geom_errorbar(aes(x = lastSampleGT1, ymin = meanGR, ymax = ymax), width = 0.2) +
 #    geom_col() +
 #    theme_publication() +    
 #    facet_grid(riverGG~seasonGG , scales = "free")
 
   ggplot(lastSampleGT1_data |> filter(n > 2, !is.na(lastSampleGT1)), aes(lastSampleGT1, meanGR, fill = speciesGG, color = speciesGG)) +
    geom_errorbar(aes(x = lastSampleGT1, ymin = meanGR, ymax = ymax), width = 0.2, position = position_dodge(0.9)) +
    geom_col(position = "dodge") +
    theme_publication() +    
    facet_grid(riverGG~seasonGG , scales = "free")
