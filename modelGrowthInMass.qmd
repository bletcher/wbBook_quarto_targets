...
predictSpp <- function(dIn, modIn, speciesIn){
  ...
  byAIC = modIn |> 
    unnest(glanced) |>
    arrange(AIC) |>
    dplyr::select(model, AIC, df.residual, fit, tidied, summary, anova)
...
predBKT$byAIC[1,]$summary
predBKT$byAIC[1,]$anova

ggplot(predBKT$pred |> filter(meanTempZbySeason %in% c(-1.5,0,1.5), meanFlowByRiverZbySeason %in% c(-1.5,0,1.5), season > 1), aes(observedWeightZ, pred, group = groupF)) +
...
ggplot(predBKT$pred |> filter(meanTempZbySeason %in% c(-1.5,0,1.5), observedWeightZ %in% c(-1.5,0,1.5), season > 1), aes(meanFlowByRiverZbySeason, pred, group = groupTM)) +
  ...
  theme_publication() +    
  facet_grid(seasonGG + observedWeightZ ~ riverGG)
...
predBNT$byAIC[1,]$summary
predBNT$byAIC[1,]$anova

ggplot(predBNT$pred |> filter(meanTempZbySeason %in% c(-1.5,0,1.5), meanFlowByRiverZbySeason %in% c(-1.5,0,1.5), season > 1, nIndBySpeciesZ == 0), aes(observedWeightZ, pred, group = groupF)) +
  geom_line(aes(color = factor(meanFlowByRiverZbySeason))) +
  labs(y = "Probability of negative growth in mass", x = "Body mass (mg)") +
  theme_publication() +    
  facet_grid(seasonGG ~ riverGG + meanTempZbySeason)

ggplot(predBNT$pred |> filter(meanTempZbySeason %in% c(-1.5,0,1.5), observedWeightZ %in% c(-1.5,0,1.5), season > 1, nIndBySpeciesZ == 0), aes(meanFlowByRiverZbySeason, pred, group = groupTM)) +
  geom_line(aes(color = factor(meanTempZbySeason))) + #, linetype = factor(observedWeightZ))) +
  labs(y = "Probability of negative growth in mass", x = "Stream flow") +
  theme_publication() +    
  facet_grid(seasonGG + observedWeightZ ~ riverGG)

ggplot(predBNT$pred |> filter(meanTempZbySeason %in% c(-1.5,0,1.5), observedWeightZ %in% c(-1.5,0,1.5), season > 1, meanFlowByRiverZbySeason == 0), aes(nIndBySpeciesZ, pred, group = groupTM)) +
  geom_line(aes(color = factor(meanTempZbySeason))) + #, linetype = factor(observedWeightZ))) +
  labs(y = "Probability of negative growth in mass", x = "Number of brown trout") +
  theme_publication() +    
  facet_grid(seasonGG + observedWeightZ ~ riverGG)
...
predATS$byAIC[1,]$summary
predATS$byAIC[1,]$anova

ggplot(predATS$pred |> filter(meanTempZbySeason %in% c(-1.5,0,1.5), meanFlowByRiverZbySeason %in% c(-1.5,0,1.5), season > 1, nIndBySpeciesZ == 0), aes(observedWeightZ, pred, group = groupF)) +
  geom_line(aes(color = factor(meanFlowByRiverZbySeason))) +
  labs(y = "Probability of negative growth in mass", x = "Body mass (mg)") +
  theme_publication() +    
  facet_grid(seasonGG ~ riverGG + meanTempZbySeason)

ggplot(predATS$pred |> filter(meanTempZbySeason %in% c(-1.5,0,1.5), observedWeightZ %in% c(-1.5,0,1.5), season > 1, nIndBySpeciesZ == 0), aes(meanFlowByRiverZbySeason, pred, group = groupTM)) +
  geom_line(aes(color = factor(meanTempZbySeason))) + #, linetype = factor(observedWeightZ))) +
  labs(y = "Probability of negative growth in mass", x = "Stream flow") +
  theme_publication() +    
  facet_grid(seasonGG + observedWeightZ ~ riverGG)

ggplot(predATS$pred |> filter(meanTempZbySeason %in% c(-1.5,0,1.5), observedWeightZ %in% c(-1.5,0,1.5), season > 1, meanFlowByRiverZbySeason == 0), aes(nIndBySpeciesZ, pred, group = groupTM)) +
  geom_line(aes(color = factor(meanTempZbySeason))) + #, linetype = factor(observedWeightZ))) +
  labs(y = "Probability of negative growth in mass", x = "Number of brown trout") +
  theme_publication() +    
  facet_grid(seasonGG + observedWeightZ ~ riverGG)
