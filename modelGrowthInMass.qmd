...
# compare with Josie's negative growth rates
# 
# josie <- data.frame(gr = c(-0.002189076,0.001439023,0.000927903,-0.001256159,-0.003833329,-0.003333316,-0.002460802))
# 
# ggplot(cd1 |> filter(speciesGG == "Brook trout", seasonGG == "Summer", riverGG == "West Brook", grWeight < 0), aes(grWeight)) +
#   geom_freqpoly(aes(group = factor(year), color = factor(year)), bins = 10) +
#   #geom_freqpoly(data = josie, aes(gr), bins = 10, color = "black", linewidth = 3) +
#   geom_dotplot(data = josie, aes(x = gr), bins = 10, color = "black") +
#   theme_publication() + 
#   theme(axis.text.x = element_text(vjust = 0.5, angle = 45), legend.key = none) +
#   labs(x = "Instantaneous growth rate in mass (mg/d)", y = "Frequency") +
#   facet_grid(seasonGG~riverGG)
...
plotGrLenMass <- function(d, s){
  ggplot(d |> filter(species == s), aes(grLength, grWeight, color = factor(year))) +
...
plotGrLenMassBySeason <- function(d, speciesIn, seasonIn){
  ggplot(d |> filter(species == speciesIn, seasonGG == seasonIn), aes(grLength, grWeight)) +
    geom_point(alpha = 0.5) +
    geom_vline(xintercept = 0) +
...
plotGrLenMassBySeason(cd1, "bkt", "Spring")
plotGrLenMassBySeason(cd1, "bkt", "Summer")
plotGrLenMassBySeason(cd1, "bkt", "Autumn")
plotGrLenMassBySeason(cd1, "bkt", "Winter")
```
...
plotGrLenMassBySeason(cd1, "bnt", "Spring")
plotGrLenMassBySeason(cd1, "bnt", "Summer")
plotGrLenMassBySeason(cd1, "bnt", "Autumn")
plotGrLenMassBySeason(cd1, "bnt", "Winter")
```
...
plotGrLenMassBySeason(cd1, "ats", "Spring")
plotGrLenMassBySeason(cd1, "ats", "Summer")
plotGrLenMassBySeason(cd1, "ats", "Autumn")
plotGrLenMassBySeason(cd1, "ats", "Winter")
```
:::

```{r}
#| label: get_intercepts

getIntercepts <- function(d){
  library(broom)
  
  d |> 
    group_by(speciesGG, seasonGG, year) |> 
    nest() |> 
    mutate(
      model = map(data, ~{
        if(nrow(.x) >= 10) {
          tryCatch(
            lm(grWeight ~ grLength, data = .x),
    ...
    dplyr::select(speciesGG, seasonGG, year, tidy_model) |> 
    unnest(tidy_model) |> 
    pivot_wider(names_from = term, values_from = estimate) |>
    rename(intercept = `(Intercept)`, slope = grLength) 
    #filter(term == "(Intercept)") |> 
    #select(speciesGG, seasonGG, year, intercept = estimate)
}

grLgrWIntercepts <- getIntercepts(cd1)


ggplot(grLgrWIntercepts, aes(intercept, color = seasonGG)) +
  geom_vline(xintercept = 0, color = "darkgrey") +
  geom_density(adjust = 1.5) +
  scale_color_discrete() +
  facet_wrap(~speciesGG)

ggplot(grLgrWIntercepts, aes(slope, color = seasonGG)) +
  geom_vline(xintercept = 0, color = "darkgrey") +
  geom_density(adjust = 1.5) +
  scale_color_discrete() +
...
ggplot(cd1 |> filter(!is.na(negGrowth), riverGG == "West Brook", seasonGG %in% c("Summer", "Autumn")), aes(observedWeight, negGrowth*1, color = speciesGG, fill = speciesGG)) +
  ...
  theme_publication() +  
  labs(x = "Fish mass (g)", y = "Probability of negative growth") +
  scale_color_grey() +
  scale_fill_grey() +
  xlim(c(0,200)) +
  facet_wrap(~seasonGG)
...
## Mean growth rates losers_gainers
### Overall
```{r}
#| label: losers_gainers
...
### By Season/species
```{r}
#| label: losers_gainers_season_species
...
propNegSRsN <- propNegSRsN |> filter(!(seasonGG == "Summer" & mTZ <0))

```

### Flow/temperature data
```{r}
#| label: propModBKT_flow_temp

ggplot(propNegSRsN, aes(mTZ, mF_log10Z, color = riverGG), shape = riverGG) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Standardized stream temperature", y = "Standardized stream flow") +
  theme_publication(base_size = 20) +
...
#### By age only, not year

```{r}
#| label: negGrowth_seenAgainAgeBoth_cd

# grouped by age
negGr_seenNotSeenAgeBothCD = cd |> 
  mutate(age = year - cohort) |>
  addGG() |>
 # filter(samplesBeforeLast > 0) |>
  group_by(speciesGG, riverGG, seasonGG, lastInterval = maxSample == sampleNumber + 1, age, isYOY) |> 
  summarize(
...
negGr_seenNotSeenAgeBothCD_wide <- negGr_seenNotSeenAgeBothCD |> ungroup() |> select(-meanGR) |> spread(lastInterval, n, fill = 0) |>
  mutate(
    ratio = `TRUE`/`FALSE`,    
    propNotSeenAgain = `TRUE`/(`FALSE` + `TRUE`),
    #gainedLost = ifelse(negGrowth, "Lost", "Gained"),
    growthYear = ifelse(isYOY, "0/1", "1+"),
    n = `FALSE` + `TRUE`
  )

ggplot(negGr_seenNotSeenAgeBothCD_wide |> filter(speciesGG == "Brook trout", age %in% c(0,1,2), 
                                                 #seasonGG %in% c("Summer", "Autumn")
                                                 ), aes(age, propNotSeenAgain)) +
  geom_col(position = "dodge") +
  #geom_hline(yintercept = 1, linetype = "dashed") +
  geom_text(aes(label = n), vjust = 0, nudge_y = +0.075, check_overlap = TRUE, size = 4) +
  labs(y = "Number of fish not seen again / number of fish seen again", x = "Age") +
  theme_publication(base_size = 18) +    
  facet_grid(seasonGG~riverGG)


#Not grouped by age/isYOY

negGr_seenNotSeenAgeBothNoAgeCD = cd |> 
  mutate(age = year - cohort) |>
  addGG() |>
 # filter(samplesBeforeLast > 0) |>
  group_by(speciesGG, riverGG, seasonGG, lastInterval = maxSample == sampleNumber + 1) |> 
  summarize(
    meanGR = mean(grWeight, na.rm = TRUE),
    n = n()
  ) |> 
...
negGr_seenNotSeenAgeBothNoAgeCD_wide <- negGr_seenNotSeenAgeBothNoAgeCD |> ungroup() |> select(-meanGR) |> spread(lastInterval, n, fill = 0) |>
  mutate(
    ratio = `TRUE`/`FALSE`, 
    n = `FALSE` + `TRUE`,
    propNotSeenAgain = `TRUE`/n,
    #gainedLost = ifelse(negGrowth, "Lost", "Gained"),
    #growthYear = ifelse(isYOY, "0/1", "1+"),
    
  )

ggplot(negGr_seenNotSeenAgeBothNoAgeCD_wide |> filter(
  #speciesGG == "Brook trout", 
  #seasonGG %in% c("Summer", "Autumn")
  ), aes(seasonGG, propNotSeenAgain, group = speciesGG, fill = speciesGG)) +
  geom_col(position = "dodge") +
  #geom_hline(yintercept = 1, linetype = "dashed") +
  geom_text(aes(label = n), vjust = 0, nudge_y = +0.075, check_overlap = TRUE, size = 4) +
  labs(y = "Number of fish not seen again / number of fish seen again", x = "Age") +
...
ggplot(negGr_seenNotSeenAgeBothNoAgeCD_wide |> filter(
  #speciesGG == "Brook trout", 
  #seasonGG %in% c("Summer", "Autumn")
  ), aes(speciesGG, propNotSeenAgain, group = riverGG, fill = riverGG)) +
  geom_col(position = "dodge") +
  #geom_hline(yintercept = 1, linetype = "dashed") +
  geom_text(aes(label = n), vjust = 0, nudge_y = +0.075, check_overlap = TRUE, size = 4) +
  labs(y = "Number of fish not seen again / number of fish seen again", x = "Age") +
...
By negGrowth
```{r}
#| label: negGrowth_seenAgainAge_cd
...
negGr_seenNotSeenCD = cd |>
  mutate(age = year - cohort) |>
  filter(age < 3) |>
  addGG() |>
 # filter(samplesBeforeLast > 0) |>
  group_by(speciesGG, riverGG, seasonGG, lastInterval = maxSample == sampleNumber + 1, negGrowth = grWeight < 0) |> 
  summarize(
    meanGR = mean(grWeight, na.rm = TRUE),
    n = n()
  ) |> 
...
negGr_seenNotSeenCD_wide <- negGr_seenNotSeenCD |> ungroup() |> dplyr::select(-meanGR) |> spread(lastInterval, n, fill = 0) |>
  mutate(
    ratio = `TRUE`/`FALSE`, 
    propNotSeenAgain = `TRUE`/(`FALSE` + `TRUE`),
    gainedLost = ifelse(negGrowth, "Negative", "Positive"),
    #growthYear = ifelse(isYOY, "0/1", "1+"),
    n = `FALSE` + `TRUE`
  )

# Figure 7 in the MS
ggplot(negGr_seenNotSeenCD_wide |> filter(speciesGG == "Brook trout", seasonGG %in% c("Summer", "Autumn")), aes(gainedLost, ratio)) +
  geom_col(position = "dodge") +
  geom_hline(yintercept = 1, linetype = "dashed") +
  geom_text(aes(label = n), vjust = 0, nudge_y = +0.075, check_overlap = TRUE, size = 4) +
  labs(y = "Number of fish not seen again / number of fish seen again", x = "Growth in mass") +
...
ggplot(negGr_seenNotSeenCD_wide |> filter(seasonGG %in% c("Summer","Autumn"), riverGG == "West Brook"), aes(gainedLost, ratio)) +
  geom_col(position = "dodge") +
  geom_hline(yintercept = 1, linetype = "dashed") +
  geom_text(aes(label = n), vjust = 0, nudge_y = +0.075, check_overlap = TRUE, size = 4) +
  labs(y = "Number of fish not seen again / number of fish seen again", x = "Growth in mass") +
...
#### By age only, not year for fish LENGTH
```{r}
#| label: negGrowthLen_seenAgainAge_cd
...
```{r}
#| label: negGrowth_seenAgainYear_cd
...
#### By age
```{r}
#| label: negGrowth_seenAgainYearAge_cd
