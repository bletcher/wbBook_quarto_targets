...
## Body size effect on prop neg growth
```{r}
#| label: propNegSize
...
ggplot(propNegSRS |> filter(n > 50), aes(seasonGG, propNeg, color = riverGG, group = riverGG)) +
  ...
  scale_color_discrete() +
  theme_publication() +  
  theme(axis.text.x = element_text(angle = 360-45, vjust = 0, hjust = .5)) +
...
### XGBoost model for proportion of fish with neg growth  
#### Brook trout
```{r}
#| label: xgBoostPropNeg

# http://uc-r.github.io/gbm_regression

dML_propNeg <- propNegSRsN |> 
  filter(speciesGG == "Brook trout", year > 2001) |> 
  dplyr::select(riverGG, year, mT, mF_log10, nInd, propNeg) 
...
```{r}
#| label: runML1_propNeg
#| 
# reproducibility
set.seed(123)

xgb.fit1_propNeg <- xgb.cv(
...
xgb.fit1_propNeg$evaluation_log %>%
  dplyr::summarise(
    ntrees.train = which(train_rmse_mean == min(train_rmse_mean))[1],
    rmseloss.train = min(train_rmse_mean),
    ntrees.test = which(test_rmse_mean == min(test_rmse_mean))[1],
    rmseloss.test = min(test_rmse_mean),
  )

# plot error vs number trees
ggplot(xgb.fit1_propNeg$evaluation_log) +
...
xgb.fit2_propNeg$evaluation_log %>%
  dplyr::summarise(
    ntrees.train = which(train_rmse_mean == min(train_rmse_mean))[1],
    rmseloss.train   = min(train_rmse_mean),
    ntrees.test  = which(test_rmse_mean == min(test_rmse_mean))[1],
...
# plot error vs number trees
ggplot(xgb.fit2_propNeg$evaluation_log) +
  geom_line(aes(iter, train_rmse_mean), color = "red") +
  geom_line(aes(iter, test_rmse_mean), color = "blue")
...
```{r}
#| label: vipBinary_propNeg

# create importance matrix
importance_matrix_propNeg <- xgb.importance(model = xgb.fit.final_propNeg)

# variable importance plot
xgb.plot.importance(importance_matrix_propNeg, top_n = 10, measure = "Gain")
...
```{r}
#| label: pdpBinary_propNeg

pFlow_propNeg <- xgb.fit.final_propNeg %>%
  pdp::partial(pred.var = "mF_log10", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) %>%
  autoplot(rug = TRUE, train = features_train_propNeg) +
  geom_smooth(se = FALSE)
  #scale_y_discrete() +
  #ggtitle("PDP")
pTemp_propNeg <- xgb.fit.final_propNeg %>%
  pdp::partial(pred.var = "mT", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) %>%
  autoplot(rug = TRUE, train = features_train_propNeg) +
  geom_smooth(se = FALSE)

pNInd_propNeg <- xgb.fit.final_propNeg %>%
  pdp::partial(pred.var = "nInd", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) %>%
  autoplot(rug = TRUE, train = features_train_propNeg) +
  geom_smooth(se = FALSE)

pYear_propNeg <- xgb.fit.final_propNeg %>%
  pdp::partial(pred.var = "year", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) %>%
  autoplot(rug = TRUE, train = features_train_propNeg) +
  geom_smooth(se = FALSE)

gridExtra::grid.arrange(pFlow_propNeg, pTemp_propNeg, pNInd_propNeg, pYear_propNeg, nrow = 3)

# #Seasons
# pSpring_propNeg <- xgb.fit.final_propNeg %>%
#   pdp::partial(pred.var = "seasonGG_lev_x_Spring", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) %>%
#   autoplot(rug = TRUE, train = features_train_propNeg)
# 
# pSummer_propNeg <- xgb.fit.final_propNeg %>%
#   pdp::partial(pred.var = "seasonGG_lev_x_Summer", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) %>%
#   autoplot(rug = TRUE, train = features_train_propNeg)
# 
# pAutumn_propNeg <- xgb.fit.final_propNeg %>%
#   pdp::partial(pred.var = "seasonGG_lev_x_Autumn", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) %>%
#   autoplot(rug = TRUE, train = features_train_propNeg)
# 
# pWinter_propNeg <- xgb.fit.final_propNeg %>%
#   pdp::partial(pred.var = "seasonGG_lev_x_Winter", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) %>%
#   autoplot(rug = TRUE, train = features_train_propNeg)
# 
# gridExtra::grid.arrange(pSpring_propNeg, pSummer_propNeg, pAutumn_propNeg, pWinter_propNeg, nrow = 2)
# 
# # All seasons on one graph
# partialSpring <- xgb.fit.final_propNeg %>%
#   pdp::partial(pred.var = "seasonGG_lev_x_Spring", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) |> 
#   mutate(seasonGG = "Spring") |> 
#   rename(x = "seasonGG_lev_x_Spring")
# 
# partialSummer <- xgb.fit.final_propNeg %>%
#   pdp::partial(pred.var = "seasonGG_lev_x_Summer", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) |> 
#   mutate(seasonGG = "Summer") |> 
#   rename(x = "seasonGG_lev_x_Summer")
# 
# partialAutumn <- xgb.fit.final_propNeg %>%
#   pdp::partial(pred.var = "seasonGG_lev_x_Autumn", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) |> 
#   mutate(seasonGG = "Autumn") |> 
#   rename(x = "seasonGG_lev_x_Autumn")
# 
# partialWinter <- xgb.fit.final_propNeg %>%
#   pdp::partial(pred.var = "seasonGG_lev_x_Winter", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) |> 
#   mutate(seasonGG = "Winter") |> 
#   rename(x = "seasonGG_lev_x_Winter")
# 
# 
# partialSeasons <- bind_rows(partialSpring, partialSummer, partialAutumn, partialWinter) |> 
#   filter(x %in% c(0,1)) |> 
#   mutate(seasonEffect = ifelse(x == 1, "Yes", "No"),
#          seasonOrdered = factor(seasonGG, levels = c("Spring", "Summer", "Autumn", "Winter"), 
#                              ordered = T)
#          )
# 
# ggplot(partialSeasons, aes(seasonOrdered, yhat, fill = factor(seasonEffect))) +
...
############################################################
#Rivers
# pWB_propNeg <- xgb.fit.final_propNeg %>%
#   pdp::partial(pred.var = "riverGG_lev_x_West_Brook", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) %>%
#   autoplot(rug = TRUE, train = features_train_propNeg)
# 
# pOL_propNeg <- xgb.fit.final_propNeg %>%
#   pdp::partial(pred.var = "riverGG_lev_x_Open_Large", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) %>%
#   autoplot(rug = TRUE, train = features_train_propNeg)
# 
# pOS_propNeg <- xgb.fit.final_propNeg %>%
#   pdp::partial(pred.var = "riverGG_lev_x_Open_Small", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) %>%
#   autoplot(rug = TRUE, train = features_train_propNeg)
# 
# pIS_propNeg <- xgb.fit.final_propNeg %>%
#   pdp::partial(pred.var = "riverGG_lev_x_Isolated_Small", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) %>%
#   autoplot(rug = TRUE, train = features_train_propNeg)
# 
# gridExtra::grid.arrange(pWB_propNeg, pOL_propNeg, pOS_propNeg, pIS_propNeg, nrow = 2)

# Al river on one graph
partialWB <- xgb.fit.final_propNeg %>%
  pdp::partial(pred.var = "riverGG_lev_x_West_Brook", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) |> 
  mutate(riverGG = "West Brook") |> 
  rename(x = "riverGG_lev_x_West_Brook")

partialOL <- xgb.fit.final_propNeg %>%
  pdp::partial(pred.var = "riverGG_lev_x_Open_Large", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg)|> 
  mutate(riverGG = "Open Large") |> 
  rename(x = "riverGG_lev_x_Open_Large")

partialOS <- xgb.fit.final_propNeg %>%
  pdp::partial(pred.var = "riverGG_lev_x_Open_Small", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg) |> 
  mutate(riverGG = "Open Small") |> 
  rename(x = "riverGG_lev_x_Open_Small")

partialIS <- xgb.fit.final_propNeg %>%
  pdp::partial(pred.var = "riverGG_lev_x_Isolated_Small", n.trees = 1082, grid.resolution = 100, train = features_train_propNeg)|> 
  mutate(riverGG = "Isolated Small") |> 
  rename(x = "riverGG_lev_x_Isolated_Small")


partialRivers <- bind_rows(partialWB, partialOL, partialOS, partialIS) |> 
  filter(x %in% c(0,1)) |> 
  mutate(RiverEffect = ifelse(x ==1, "Yes", "No"),
         riverOrdered = factor(riverGG, levels = c("West Brook", "Open Large", "Open Small", "Isolated Small"), 
                             ordered = T))

ggplot(partialRivers, aes(riverOrdered, yhat, fill = factor(RiverEffect))) +
  geom_col(position = "dodge", size = 1.5) +
  labs(y = "Effect on proportion of negative observations")
...
#### hstats
https://github.com/mayer79/hstats#background
```{r}
#| label: hstatsPropNegPropNeg

s_propNeg <- hstats(xgb.fit.final_propNeg, v = new_vars_propNeg, X = features_train_propNeg)
s_propNeg
plot(s_propNeg)

# top 5 2-way interactions
h2_pairwise(s_propNeg, normalize = FALSE, squared = FALSE, top_m = 8)

# 3-way interactions
plot(s_propNeg, which = 1:3, normalize = F, squared = F, facet_scales = "free_y", ncol = 1)
```

Interaction plots
```{r}
#| label: hstatsInteractionsPlots_propNeg

plot(partial_dep(xgb.fit.final_propNeg, v = "mT", X = features_train_propNeg, BY = "mF_log10")) +
  geom_smooth(aes(color = mF_log10), se = FALSE)

plot(partial_dep(xgb.fit.final_propNeg, v = "mF_log10", X = features_train_propNeg, BY = "mT")) +
  geom_smooth(aes(color = mT), se = FALSE)

plot(partial_dep(xgb.fit.final_propNeg, v = "mF_log10", X = features_train_propNeg, BY = c("nInd"))) +
  geom_smooth(aes(color = nInd), se = FALSE)
#plot(partial_dep(xgb.fit.final, v = c("observedWeight", "meanFlowByRiver"), X = features_train, grid_size = 1000))

# plot(ice(xgb.fit.final_propNeg, v = "mF_log10", X = features_train_propNeg, BY = c("mT", "seasonGG_lev_x_Summer")), center = FALSE) +
#   geom_smooth(aes(color = mT), se = FALSE) +
#   scale_color_viridis_b()


plot(ice(xgb.fit.final_propNeg, v = "mF_log10", X = features_train_propNeg, BY = c("mT", "riverGG_lev_x_Isolated_Small")), center = FALSE)
```

Variable importance plots
```{r}
#| label: hstatsVIPlots_propNeg

pd_importance(s_propNeg)

# Compared with tree split gain importance
xgb.plot.importance(xgb.importance(model = xgb.fit.final_propNeg), measure = "Gain")
```
...
#Seasons
pSpring <- xgb.fit.final %>%
  pdp::partial(pred.var = "seasonGG_lev_x_Spring", n.trees = 1082, grid.resolution = 100, train = features_train) %>%
  autoplot(rug = TRUE, train = features_train)

pSummer <- xgb.fit.final %>%
  pdp::partial(pred.var = "seasonGG_lev_x_Summer", n.trees = 1082, grid.resolution = 100, train = features_train) %>%
  autoplot(rug = TRUE, train = features_train)

pAutumn <- xgb.fit.final %>%
  pdp::partial(pred.var = "seasonGG_lev_x_Autumn", n.trees = 1082, grid.resolution = 100, train = features_train) %>%
  autoplot(rug = TRUE, train = features_train)

pWinter <- xgb.fit.final %>%
  pdp::partial(pred.var = "seasonGG_lev_x_Winter", n.trees = 1082, grid.resolution = 100, train = features_train) %>%
  autoplot(rug = TRUE, train = features_train)

gridExtra::grid.arrange(pSpring, pSummer, pAutumn, pWinter, nrow = 2)
...
s <- hstats(xgb.fit.final, v = new_vars, X = features_train)
s
plot(s)

# top 5 2-way interations
...
Interaction plots
```{r}
#| label: hstatsInteractionsPlots

plot(partial_dep(xgb.fit.final, v = "observedWeight", X = features_train, BY = "meanFlowByRiver"))

#plot(partial_dep(xgb.fit.final, v = c("observedWeight", "meanFlowByRiver"), X = features_train, grid_size = 1000))

plot(ice(xgb.fit.final, v = "observedWeight", X = features_train, BY = "meanFlowByRiver"), center = FALSE)
plot(ice(xgb.fit.final, v = "meanFlowByRiver", X = features_train, BY = c("riverGG_lev_x_West_Brook", "seasonGG_lev_x_Summer")), center = FALSE)
```
...
# Compared with tree split gain importance
xgb.plot.importance(xgb.importance(model = xgb.fit.final), measure = "Gain")
```
