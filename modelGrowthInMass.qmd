...
ggplot(cd1 |> filter(seasonGG == "Summer", !is.na(negGrowth)), aes(distMoved, color = negGrowth)) +
  geom_density(adjust = 8) +
  ggtitle("All species, summer") +
  theme_publication() +    
  facet_wrap(~speciesGG)#, scales = "free")

modSummer <- (lm(distMoved ~ negGrowth * species, data = cd1 |> filter(seasonGG == "Summer", !is.na(negGrowth))))
summary(modSummer)
anova(modSummer)

ggplot(cd1 |> filter(seasonGG == "Autumn", !is.na(negGrowth)), aes(distMoved, color = negGrowth)) +
  geom_density(adjust = 8) +
  ggtitle("All species, autumn") +
  theme_publication() +    
  facet_wrap(~speciesGG)#, scales = "free")

modAutumn <- (lm(distMoved ~ negGrowth * species, data = cd1 |> filter(seasonGG == "Autumn", !is.na(negGrowth))))
summary(modAutumn)
anova(modAutumn)
