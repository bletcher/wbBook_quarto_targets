...
dGAM = cd1 |> 
  ...
  dplyr::select(riverGG, seasonGG, speciesGG, observedWeight, cohort, age, cohortF, ageF, 
                meanTemperatureScaledBySeason, meanFlowByRiverScaledBySeason, 
                meanTemperatureScaledBySeasonRiver, meanFlowByRiverScaledBySeasonRiver,grWeight) |> 
  rename(tempS = meanTemperatureScaledBySeason, flowS = meanFlowByRiverScaledBySeason,
         tempSR = meanTemperatureScaledBySeasonRiver, flowSR = meanFlowByRiverScaledBySeasonRiver) |> 
  filter(tempSR > -4)

kValue <- 4 

ggplot(dGAM |> filter(speciesGG != "Atlantic salmon"), aes(flowS, grWeight, group = speciesGG, color = speciesGG)) +
  geom_point(alpha = 0.2) +
...
ggplot(dGAM |> filter(speciesGG != "Atlantic salmon"), aes(flowSR, grWeight, group = speciesGG, color = speciesGG)) +
...
runLinearModels <- function(d, speciesIn) {
  models =
    tibble(model = 
      c(
          'grWeight ~ (riverGG * seasonGG * tempSR * flowSR * observedWeight + cohortF * ageF)',
          'grWeight ~ (riverGG * seasonGG * tempSR * flowSR * observedWeight + cohortF)',
          'grWeight ~ (riverGG * seasonGG * tempSR * flowSR + cohortF * ageF)',
          'grWeight ~ (riverGG * seasonGG * tempSR * flowSR + cohortF)',
          'grWeight ~ (riverGG * seasonGG + tempSR * flowSR + cohortF * ageF)',
          'grWeight ~ (riverGG * seasonGG + tempSR + flowSR + cohortF * ageF)'
       )
    ) |>
  ...
  models =
    tibble(model = 
      c(
            ...
            'grWeight ~ riverGG * seasonGG * observedWeight * tempSR * flowSR + cohortF * ageF +             
               s(flowSR, bs = "cr", k = kIn, by = riverGG) +
               s(flowSR, bs = "cr", k = kIn, by = seasonGG) +
               s(tempSR, bs = "cr", k = kIn, by = riverGG) +
               s(tempSR, bs = "cr", k = kIn, by = seasonGG)'
