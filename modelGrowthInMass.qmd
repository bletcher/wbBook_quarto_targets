...
negGr_seenNotSeenAgeCD = cd |> 
 ...
 # filter(samplesBeforeLast > 0) |>
  group_by(speciesGG, riverGG, seasonGG, lastInterval = maxSample == sampleNumber + 1, negGrowth = grWeight < 0, age) |> 
  summarize(
...
ggplot(negGr_seenNotSeenAgeCD_wide |> filter(riverGG == "West Brook", age %in% c(1,2), seasonGG %in% c("Summer", "Autumn")), aes(gainedLost, ratio)) +
  geom_col(position = "dodge") +
  #geom_errorbar(aes(ymin = ymin, ymax = ymax), position = position_dodge(0.9), width = 0.2) +
  geom_hline(yintercept = 1, linetype = "dashed") +
  #labs(y = "Ratio of fish not seen again to fish seen again", x = "Growth in mass") +
