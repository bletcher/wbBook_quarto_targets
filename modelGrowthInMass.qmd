...
runXGB <- function(d = propNegSRsN, vars = c("speciesGG", "riverGG", "year", "mT", "mF_log10", "nInd", "propNeg"), nRoundsIn = 100000) {
...
ggplot(all_propNegDataBKT, aes(xScaled, yhat, color = var)) +
...
gridExtra::grid.arrange(pFlow_propNegATS, pTemp_propNegATS, pNInd_propNegATS, pYear_propNegATS, nrow = 3)
...
ggplot(viAll |> filter(Feature %in% c("mF_log10", "mT", "nInd", "year")), aes(feature2, Gain, fill = sppOrdered)) +
  ...
  theme_publication() +
  scale_fill_grey() +
  labs(y = "Effoct on proportion of negative observations", x = "Variable")

########################################################

all_propNegData3spp <- bind_rows(all_propNegDataBKT, all_propNegDataBNT, all_propNegDataATS)

ggplot(all_propNegData3spp |> filter(var == "Flow"), aes(xScaled, yhat, color = speciesGG)) +
  #geom_point(alpha = 0.2) +
  geom_smooth(se = FALSE) +
  theme_publication() +
  labs(y = "Effect on proportion of negative observations", x = "Scaled flow")


ggplot(all_propNegData3spp |> filter(var == "Temperature"), aes(xScaled, yhat, color = speciesGG)) +
  #geom_point(alpha = 0.2) +
  geom_smooth(se = FALSE) +
  theme_publication() +
  labs(y = "Effect on proportion of negative observations", x = "Scaled temperature")

ggplot(all_propNegData3spp |> filter(var == "Num inds"), aes(xScaled, yhat, color = speciesGG)) +
  #geom_point(alpha = 0.2) +
  geom_smooth(se = FALSE) +
  theme_publication() +
  labs(y = "Effect on proportion of negative observations", x = "Scaled Number of individuals")

ggplot(all_propNegData3spp |> filter(var == "Year"), aes(xScaled, yhat, color = speciesGG)) +
  #geom_point(alpha = 0.2) +
  geom_smooth(se = FALSE) +
  theme_publication() +
  labs(y = "Effect on proportion of negative observations", x = "Scaled year")
