...
#|
  (summary_tt_growth <- MCMCsummary(object = out_NN_OBmod1$mcmc, params = c("grIntT"), round = 3) %>%
  ...
  #summary_tt_growth_z0 <- MCMCSummaryRMNA(object = out_NN_OBmod1$mcmc, params = c("weightDATA"), round = 3) %>%
  summary_tt_growth_zGR <- MCMCSummaryRMNA(object = out_NN_OBmod1$mcmc, params = c("gr"), round = 3) %>%
    rownames_to_column(var = "var") |> 
    mutate(
  ...
  summary_tt_growth_zW <- MCMCSummaryRMNA(object = out_NN_OBmod1$mcmc, params = c("weightDATA"), round = 3) %>%
    rownames_to_column(var = "var") |> 
    mutate(
      ind0 = str_match(var, "([0-9]+), ([0-9]+)")[,2],
      t0 =   str_match(var, "([0-9]+), ([0-9]+)")[,3],
  ...
  summary_tt_growth_z0 <- summary_tt_growth_zW |> 
    left_join(summary_tt_growth_zGR, by = c("ind", "t"), suffix = c("_weight", "_gr"))
```
...
#| label: saveCSV
#| 
  write.csv(summary_tt_growth_zR, './data/outForObservable/summary_tt_growth_zR.csv')
```
...
Plot.plot({
  ...
  marks: [
    Plot.frame(),
    Plot.dot(summary_tt_growth_zR_selected, {
      x: "t",
      y: "mean_weight"
    }),
    Plot.line(summary_tt_growth_zR_selected, {
      x: "t",
      y: "mean_weight"
    }),
    Plot.dot(summary_tt_growth_zR_selected, {
    ...
    Plot.ruleX(summary_tt_growth_zR_selected, {
      x: "first",
      y: 3,
      stroke: "green"
    }),
  ...
  facet: {
    data: summary_tt_growth_zR_selected,
    x: "ind"
  }
})
```
