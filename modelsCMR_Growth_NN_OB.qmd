...
#|
  ...
  summary_tt_growth_z0 <- MCMCSummaryRMNA(object = out_NN_OBmod1$mcmc, params = c("gr", "weightDATA"), round = 3) %>%
    rownames_to_column(var = "varAll") |> 
    mutate(
      ind0 = str_match(varAll, "([0-9]+), ([0-9]+)")[,2],
      t0 =   str_match(varAll, "([0-9]+), ([0-9]+)")[,3],
      ind = as.numeric(ind0),
      t = as.numeric(t0),
      var = str_split(varAll, "\\[")[[1]][1]
    ) |> 
...
Plot.plot({
  ...
  marks: [
    Plot.frame(),
    Plot.dot(summary_tt_growth_zR_OJS_T_selected.filter(d => d.var === "weightDATA"), {
      x: "t",
      y: "mean"
    }),
    Plot.line(summary_tt_growth_zR_OJS_T_selected.filter(d => d.var === "weightDATA"), {
      x: "t",
      y: "mean"
    }),
    Plot.dot(summary_tt_growth_zR_OJS_T_selected.filter(d => d.var === "gr"), {
      x: "t",
      y: "mean",
      fill: "green"
    }),
