...
```{r}
#| label: OB_tt_growth_NN_plotSurvWeight
...
## modelNum 4: phi(i,ys) * g(i,ys), p(i,t) model
...
```{r, echo=FALSE, results='asis'}
#| label: OB_tt_growth_NN_statsTable4

kable(as_tibble(out_NN_OBmod4$runData), caption = "Run statistics")

paste0('Run time = ', round(out_NN_OBmod4$runTime, 3), ' ',  attr(out_NN_OBmod4$runTime, "units"))
```

### Plot traces

```{r}
#| label: OB_tt_growth_NN_plotTraces4
...
Summary values  
```{r}
#| label: OB_tt_growth_NN_SummaryZ4
#|
(summaryMod4_tt_growth <- MCMCsummary(object = out_NN_OBmod4$mcmc, params = c("phiInt", "p", "grInt", "phiBeta"), round = 3) %>%
    rownames_to_column(var = "var"))
...
Add other variables to summary values  
```{r}
#| label: OB_tt_growth_NN_Summary24
#|  
  ehLong <- 
    eh$eh |>
    as.data.frame() |> 
...
```{r}
#| label: OB_tt_growth_NN_getResids4
...
```{r}
#| label: OB_tt_growth_NN_plotSurvWeight4
