...
put all the cohort summaries into one big database and then show/filter that
...
```{r}
#| cache: FALSE
ojs_define(numTags_OJS_mod3_2002 = dim(eh$tags)[1])
ojs_define(summary_OB_OJS_2002 = (summary_OB))
...
viewof selectInd_mod3_2002 = Inputs.select([...Array(numTags_OJS_mod3_2002).keys()], {
  label: "Which fish?",
  value: 1,
  step: 1,
  multiple: true
...
summary_OB_OJS_T_2002 = transpose(summary_OB_OJS_2002)
summary_OB_OJS_T_2002_selected = summary_OB_OJS_T_2002.filter((d) =>
  selectInd_mod3_2002.includes(d.ind)
)
...
plot_z(summary_OB_OJS_T_2002_selected)
...
```{r}
#| cache: FALSE
ojs_define(numTags_OJS_mod3_2003 = dim(eh$tags)[1])
ojs_define(summary_OB_OJS_2003 = (summary_OB))
#ojs_define(summary_tt_z_OJS = (summary_tt_z_mod3))
```
...
viewof selectInd_mod3 = Inputs.select([...Array(numTags_OJS_mod3_2003).keys()], {
  label: "Which fish?",
  value: 1,
  step: 1,
  multiple: true
...
summary_OB_OJS_T = transpose(summary_OB_OJS_2003)
summary_OB_OJS_T_selected = summary_OB_OJS_T.filter((d) =>
...
plot_z(summary_OB_OJS_T_selected)
