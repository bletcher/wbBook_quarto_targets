...
modelYOY_target <-
  tar_plan(
    cdWB_electroYOY = cdWB_electro_target,
    envDataWBYOY = envDataWB_target,
    
    firstObs_noTag_target = cdWB_electroYOY %>%
      filter(is.na(tag), ageInSamples == 1) %>%
      mutate(n = 1) %>%
      dplyr:: select(all_of(selectedVariables)),
    
    firstObs_tag_target = cdWB_electroYOY %>%
        group_by(tag) %>%
    ...
    firstObs_Env_target = firstObsDates_target %>%
      ...
      mutate(
        ...
        spawn_emerge =  list(getEnvMeans(envDataWBYOY, river, spawnDate, emergeDate)),
        emerge_detect = list(getEnvMeans(envDataWBYOY, river, emergeDate, date)),
        spawn_detect =  list(getEnvMeans(envDataWBYOY, river, spawnDate, date)),
        oneMonth =      list(getEnvMeans(envDataWBYOY, river, oneMonthDate, date)),
        threeMonth =    list(getEnvMeans(envDataWBYOY, river, threeMonthDate, date)),
        fiveMonth =     list(getEnvMeans(envDataWBYOY, river, fiveMonthDate, date))
      ),
    ...
    envDataWBFlow_target = envDataWBYOY %>%
...
getEnvMeans <- function(d, riverIn, startIn, endIn) { 

  out <- d %>% 
    filter(river == riverIn, dateDate >= startIn, dateDate <= endIn) %>%
    summarize(
      ...
      #changed flow to flowByRiver jan 2024 because should be better rep of trib flows
      sumF =   sum(flowByRiver, na.rm = TRUE),
      meanF = mean(flowByRiver, na.rm = TRUE),
      sdF =     sd(flowByRiver, na.rm = TRUE),
      cvF = sdF/meanF,
      n = n()
    )
  
  #message(paste(riverIn, startIn, endIn))
  return(out)
}
