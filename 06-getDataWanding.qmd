...
```{r wanding_yearByYear, cache=TRUE}
...
```{r}
#Jimmy on the left, Mitchell on the right
ggplot(cdWB_wandingTribs3, aes(moveRate, linetype = species)) +
  geom_freqpoly(binwidth=5,origin = min(cdWB_wandingTribs3$moveRate)) +
...
## West Brook models
```{r RE model for individual movement variation WB}
