...
Plot Xiaowei's results
```{r}
#| label: plotXiaowei

# ggplot(xP |> filter(ind %in% 1:100), aes(t, xP, color = factor(ind))) +
#   geom_line()
# 
# ggplot(xPhi |> filter(ind %in% 1:nrow(xPhi)), aes(t, xPhi)) +
#   geom_jitter(alpha = 0.33)
# 
# ggplot(xPhi |> filter(ind %in% 1000:1100), aes(t, xPhi, color = factor(ind))) +
#   geom_line()
```
...
### Compare survival with Xioawei's survival estimates
```{ojs}
