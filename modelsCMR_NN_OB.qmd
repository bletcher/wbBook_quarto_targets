...
ojs_define(summary_ss_z_OJS = (summary_tt_z))
```
...
viewof selectInd = Inputs.select([...Array(numTags_ss_OJS).keys()], {
  label: "Which fish?",
  value: 1,
  step: 1,
  multiple: true
...
Model with phi and p for each season combination. Season is hierarchical over time.
