...
ggplot(betaInts %>% filter(varName == "betaIntRiverYOYSeasonOut"), aes(isYOY, mean, color = flowData)) +
  ...
  facet_wrap(season~river, labeller = global_labellerInt_WB) +
  guides(color = guide_legend(
    title = "Data source"
  ))
```
...
|  Data added to `cdWB_CMR0_target` using addEnvironmental3() in getDataCMR_targets.R
