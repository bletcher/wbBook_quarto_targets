...
getEigen <- function(flowType, numEggs, earlySurvival, fIn1, fIn2, fIn3, fIn4, tIn){
  ...
  A <- A2%*%A1%*%A4%*%A3
  return(list(A = A, lambdas = eigen(A), A1=A1, A2=A2, A3=A3, A4=A4))
}

sens <- tibble(flowType = character(), e=numeric(), s=numeric(), fIn = numeric(), tIn = numeric(), lambda=numeric())
As <- list()
counter = 0
for(flowType in c("flowByArea", "flowByRiver")){
  for(ee in seq(80, 120, 10)){
    for(fIn1 in seq(-1.5, 1.5, 0.5)){
      for(fIn2 in seq(-1.5, 1.5, 0.5)){
        for(fIn3 in seq(-1.5, 1.5, 0.5)){
          for(fIn4 in seq(-1.5, 1.5, 0.5)){
            ss <- 0.03356
            tIn = 0
            ...
            sens <- bind_rows(sens, tibble(flowType = flowType, e=ee, s=ss, fIn=0, tIn=0, lambda = As[[counter]]$lambdas$values[1]))
          }
        }
      }
    }
...
ggplot(sens, aes(e, lambda, color = flowType)) +
  geom_point() +
