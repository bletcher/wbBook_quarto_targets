...
Flow predictions
```{r}
predFlowCohort <- getPredictionsFlowCohort_current(out_flow, everyNIters = 3) %>%
    mutate(flowData = "flow") 

predFlowByRiverCohort <- getPredictionsFlowCohort_current(out_flowByRiver, everyNIters = 3) %>%
    mutate(flowData = "flowByRiver")
...
```{r}
getPredictionsFlowCohort_current <- function(toSave, everyNIters = 10, flowStep = 0.5){
