...
# 
  ...
  MCMCtrace(object = out_flowByRiver$mcmc,
            #ISB = FALSE,
            #exact = TRUE, 
            params = c(  "betaIntTop", "betaPhiTop","betaFlowTop","betaPTop"  ),
            pdf = FALSE, 
            priors = priors)
...
```{r}
predFlowCohort <- getPredictions_FlowYOYSeasonCohort(out_flow, everyNIters = 3) %>%
    mutate(flowData = "flow") 

predFlowByRiverCohort <- getPredictions_FlowYOYSeasonCohort(out_flowByRiver, everyNIters = 3) %>%
    mutate(flowData = "flowByRiver")
...
ggplot(predFlowByRiverCohort, aes(flow, predPhi, group = (iter))) +
  geom_line(alpha = 0.05) +
  facet_grid(season + isYOY~cohort)
```
...
```{r}
predFlowYOYSeason <- getPredictions_FlowYOYSeason(out_flow, everyNIters = 3) %>%
    mutate(flowData = "flow") 

predFlowByRiverYOYSeason <- getPredictions_FlowYOYSeason(out_flowByRiver, everyNIters = 3) %>%
    mutate(flowData = "flowByRiver")
...
#### Graph flow predictions
```{r}
ggplot(predFlowYOYSeason, aes(flow, predPhi, group = (iter))) +
  geom_line(alpha = 0.05) +
  facet_grid(season ~ isYOY)


ggplot(predFlowByRiverYOYSeason, aes(flow, predPhi, group = (iter))) +
  geom_line(alpha = 0.05) +
  facet_grid(season ~ isYOY)
```
...
getPredictions_FlowYOYSeason <- function(toSave, everyNIters = 10, flowStep = 0.5){
  ...
  for(i in 1:nrow(predictorsBetaFlow)){
    predictorsBetaFlow$betaFlow1[i] <- mcmc$chain1[[predictorsBetaFlow[i, "iter"], 
                                                    paste0("betaFlowYOYSeason[1, ", predictorsBetaFlow[i, "isYOY"],
                                                           ", ",                    predictorsBetaFlow[i, "season"],
                                                           "]")
    ]]
    predictorsBetaFlow$betaFlow2[i] <- mcmc$chain1[[predictorsBetaFlow[i, "iter"], 
                                                    paste0("betaFlowYOYSeason[2, ", predictorsBetaFlow[i, "isYOY"],
                                                           ", ",                    predictorsBetaFlow[i, "season"],
                                                           "]")
    ]]
  }
...
FlowTop predictions
```{r}
predFlowTop <- getPredictionsFlowTop(out_flow, everyNIters = 3) %>%
    mutate(flowData = "flow") 

predFlowByRiverTop <- getPredictionsFlowTop(out_flowByRiver, everyNIters = 3) %>%
    mutate(flowData = "flowByRiver")
...
getPredictionsFlowCohort <- function(toSave, everyNIters = 10, flowStep = 0.5){
  ...
  predictorsBetaPhi <- expand.grid(
    iter = seq(1, dim(mcmc$chain1)[1], everyNIters),
    timeStep = 1:(toSave$myConstants$T - 1),
    cohort = 1:toSave$myConstants$nCohorts
  )
  ...
  for(i in 1:nrow(predictorsBetaPhi)){
    predictorsBetaPhi$betaPhi[i] <- mcmc$chain1[[predictorsBetaPhi[i, "iter"], 
                                                 paste0("betaPhi[", predictorsBetaPhi[i, "timeStep"],
                                                        ", ",       predictorsBetaPhi[i, "cohort"],
                                                        "]")
    ]]
  }
