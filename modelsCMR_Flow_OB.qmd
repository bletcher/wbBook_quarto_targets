...
ggplot(both %>% filter(grepl("betaFlowCohort", var)), aes(color = flowData)) +
  geom_point(aes(var, mean), size = 2, position = position_dodge(width = 0.5)) +
  geom_errorbar( aes(var, ymin = lo, ymax = hi), width = 0.4, position = position_dodge(width = 0.5)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))

ggplot(both %>% filter(grepl("betaPhiCohortOut", var)), aes(color = flowData)) +
  geom_point(aes(var, mean), size = 2, position = position_dodge(width = 0.5)) +
  geom_errorbar( aes(var, ymin = lo, ymax = hi), width = 0.4, position = position_dodge(width = 0.5)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))

ggplot(both %>% filter(grepl("betaPhiOut", var)), aes(color = flowData)) +
  geom_point(aes(var, mean), size = 2, position = position_dodge(width = 0.5)) +
  geom_errorbar( aes(var, ymin = lo, ymax = hi), width = 0.4, position = position_dodge(width = 0.5)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))

ggplot(both %>% filter(grepl("betaPCohortOut", var)), aes(color = flowData)) +
  geom_point(aes(var, mean), size = 2, position = position_dodge(width = 0.5)) +
  geom_errorbar( aes(var, ymin = lo, ymax = hi), width = 0.4, position = position_dodge(width = 0.5)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust=1))
...
Flow predictions
```{r}
predFlowCohort <- getPredictionsFlowCohort(out_flow, everyNIters = 3) %>%
    mutate(flowData = "flow") 

predFlowByRiverCohort <- getPredictionsFlowCohort(out_flowByRiver, everyNIters = 3) %>%
    mutate(flowData = "flowByRiver")
...
tS <- 10
ggplot(predFlowCohort %>% filter(timeStep == tS), aes(flow, predPhi, group = (iter))) +
  geom_line(alpha = 0.05) +
  ggtitle(paste0("timeStep (AIS) = ", tS)) +
  facet_grid(season~cohort)
